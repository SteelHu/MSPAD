    ...
    for src in files:
        for trg in files:
            if src != trg:
                ...
                command = ['python', train, '--algo_name', 'dacad', '--num_epochs', '20', '--queue_size', '98304', '--momentum', '0.99', '--batch_size', '256',
                           '--eval_batch_size', '256', '--learning_rate', '1e-4', '--dropout', '0.2', '--weight_decay', '1e-4', '--num_channels_TCN', '128-128-128',
                           '--dilation_factor_TCN', '3', '--kernel_size_TCN', '7', '--hidden_dim_MLP', '256', '--path_src', 'datasets/Boiler', '--path_trg', 'datasets/Boiler',
                           '--id_src', src, '--id_trg', trg, '--experiment_folder', 'boiler']
