...
MSL数据集CLUDA实验主入口
====================
功能：在MSL (Mars Science Laboratory) 数据集上使用CLUDA算法进行跨域异常检测实验
        ...
        for trg in ['C-1']:
            ...
            if src != trg:
                ...
                command = [
                    ...
                    '--batch_size', '256',                # 训练批次大小：256（可根据GPU内存调整）
                    ...
                    '--experiment_folder', 'MSL_cluda'   # 实验结果保存文件夹（区分CLUDA）
                ]
                
                # 执行训练（会保存模型到 results/MSL_cluda/{src}-{trg}/ 目录）
                result = subprocess.run(command)
                
                # 如果训练失败，跳过评估
                if result.returncode != 0:
                    print(f"Training failed for {src} -> {trg}, skipping evaluation.")
                    continue
                ...
                command1 = [
                    'python', test,
                    '--experiments_main_folder', 'results',  # 结果根目录
                    '--experiment_folder', 'MSL_cluda',      # 实验子目录
                    '--id_src', src,                         # 源域ID
                    '--id_trg', trg                          # 目标域ID
                ]
